...
const LogIn = () => {
    ...
    const onSubmit = data => {
        creatNewAccount ? firebase.auth().createUserWithEmailAndPassword(newUserInfo.email, newUserInfo.password)
            .then((userCredential) => {
                const newUser = { ...newUserInfo }
                newUser.error = "";
        ...
        <section className="container mt-5">
            <article className="col-md-6 mx-auto px-3">
                <div className="log-in-box border pt-3 pb-3 text-center">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="logInOrRegisterFrom">
                        ...
                        {_.get("password.type", errors) === "pattern" && (
                            <p className="error">Password must be at least 1 numaric number</p>
                        )}
                        ...
                        {creatNewAccount && <input
                            ...
                            ref={register({
                                required: true,
                                validate: value =>
                                    value === newUserInfo.password || "The passwords do not match"
                            })}
                        />}
                        {_.get("confarmPassword.type", errors) === "required" && (
                            <p className="error">This field is required</p>
                        )}
                        {errors.confarmPassword && <p className="error">{errors.confarmPassword.message}</p>}
                        {/* Start Conditional Confarm Password Filed For Sing Up */}
                    ...
                    {newUserInfo.success && <p style={{ color: "green" }}>User {creatNewAccount ? "Created" : "logedIn"} Successfully</p>}

                    {/* start Conditional change alredy have account text and functionality */}
                    {creatNewAccount ? <p className="font-weight-500 mt-3">Already have an account?<span className="custom-color custom-underline cursor-pointer" onClick={() => setCreatNewAccount(!creatNewAccount)}>Login</span> </p> : <p className="font-weight-500 mt-3">Don't have an account?<span className="custom-color custom-underline cursor-pointer" onClick={() => setCreatNewAccount(!creatNewAccount)}>Creat an Account</span> </p>}
                    {/* start Conditional change alredy have account text and functionality */}
                </div>
                <p className="form-devider px-3 m-3">or</p>

                <div className="px-3 social-singin-button">
                    <button className="text-left mb-3 border d-flex align-items-center" onClick={handelFacebookSingIn} ><FontAwesomeIcon icon={faFacebook} className="text-primary" />
                        <p>Continue with Facebook</p></button>
                    <button className="text-left d-flex align-items-center" onClick={handelGoogleSingIn}><FontAwesomeIcon icon={faGoogle} />
